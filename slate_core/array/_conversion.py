...
def as_feature_basis[
    ...
    return array.with_basis(basis.as_feature(array.basis, features))
...
def as_supports_type_basis[
    ...
    return array.with_basis(converted)
...
def as_transformed_basis[M: AnyMetadata, DT: np.dtype[np.complexfloating]](
    ...
    return array.with_basis(basis.as_transformed(array.basis))
...
def as_upcast_basis[
    ...
    return cast_basis(array, upcast)
...
def as_inner_basis[B: Basis, DT: np.dtype[np.generic]](
    ...
    # it is safe to use it in a conversion.
    # Unfortunately is not possible to express this invariant in the type system.
    return array.with_basis(array.basis.inner)
...
def as_diagonal_array[B: Basis, DT: np.dtype[np.generic]](
    ...
    # it is safe to use it in a conversion.
    # Unfortunately is not possible to express this invariant in the type system.
    return cast_basis(array, array.basis.inner.children[1])
...
def nest[
    ...
    # Unfortunately is not possible to express this invariant in the type system.
    return cast_basis(array, TupleBasis((array.basis,)))
...
def flatten[DT: np.dtype[np.generic]](
    ...
    if len(basis_as_tuple.children) == 1:
        ...
        # it is safe to call ok on the builder.
        # Unfortunately is not possible to express this invariant in the type system.
        return cast_basis(converted, basis_as_tuple.children[0])
    children = tuple(
    ...
    return cast_basis(converted, basis.flatten(basis_as_tuple))
